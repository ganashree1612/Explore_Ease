...
import Mallsitems from "./Mallsitems";
import Tab from "./tab";
import "../App.css";
...
const Gallerymalls = () => {
  ...
  const [locations, setLocations] = useState([
    ...new Set(Malls.map((item) => item.location)),
    "All",
  ]);
const [selectedItemName, setSelectedItemName] = useState();
  const filterItems = (selectedLocation, selectedPrice) => {
    if (selectedLocation === "All" && selectedPrice === "All") {
      ...
      const updatedItems = Malls.filter((curElem) => {
        return (
  ...
  return (
    <>
      ...
      <hr />
      {/* Main items section */}
      <Mallsitems items={items} handleItemClick={handleItemClick} />
      {selectedItemName && <ReviewPage placeId={selectedItemName} />}
    </>
  );
};

export default Gallerymalls;
